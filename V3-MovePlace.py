...
def update_click():
    ...
    if x >= GAME_SCREEN_WIDTH and x <= SCREEN_WIDTH-TILE_SIZE_W and y >= 200 and y <= 600: 
    ...
    for row_num, row in enumerate(piece_map):
        for column_num, column in enumerate(piece_map[row_num]):
            if piece_map[row_num][column_num]%2 == 1:
                game_map[row_num+int(piece_y/TILE_SIZE_H)][column_num+int(piece_x/TILE_SIZE_W)] = piece_map[row_num][column_num]
