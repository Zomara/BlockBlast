...
def update_click(override=False):
    ...
    elif x < GAME_SCREEN_WIDTH:
        column = int(x/TILE_SIZE_W)
        row = int(y/TILE_SIZE_H)
        if not override:
            game_map[row][column] = (game_map[row][column][0]+1, game_map[row][column][1])
        else:
            game_map[row][column] = (1, game_map[row][column][1])
...
def algorithm(i):
    ...
    if solution == None:
        ...
        game_map = copy.deepcopy(map_pos_0)
        animation(solution)

def animation(solution):
    update_map(.5)
    for piece_map, piece_x, piece_y in solution:
        waiting = True
        while waiting:
            ...
            if keys[pygame.K_z]:
                waiting = False
        place_piece(piece_map, piece_x, piece_y)
        update_map(.5)


def update_map(delay=0):
    render()
    time.sleep(delay)
    ...
    if clear_list:
        time.sleep(delay)
        for row, column in clear_list:
            game_map[row][column] = (0, (100, 100, 100))
        render()
